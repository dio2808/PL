...
class GCPBuildErrorAgent:
    """
    A helper class to create a GCP Build Error assistant agent
    using Google ADK and Gemini 2.5 Flash.
    """
    def __init__(self, name="GCP Build Error Assistant"):
        self.name = name
    ...
    def _load_document(self):
        """
        Load the GCP errors content as a string.
        Can be replaced with PDF loading or multiple documents later.
        """
        return """
Authentication / IAM:
...
"""
    ...
    def _create_agent(self):
        """
        Create the ADK agent using Gemini 2.5 Flash.
        """
        return Agent(
            name=self.name,
            instruction=(
...
agent_instance = GCPBuildErrorAgent()
agent = agent_instance.agent  # ADK requires variable named 'agent' in agents/ folder
